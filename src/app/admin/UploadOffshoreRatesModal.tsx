...
function downloadTemplate() {
  ...
  a.href = url
  a.download = "offshore-rates-template.csv"
  a.click()
  URL.revokeObjectURL(url)
}

export default function UploadOffshoreRatesModal({ targetSlug }: { targetSlug: string }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/admin/upload-offshore-rates", { method: "POST", body: fd })
        ...
        >
          <div className="relative bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-zinc-800">
              <div>
                <h2 className="text-base font-semibold text-gray-900 dark:text-zinc-50">Upload Offshore Rates</h2>
              ...
              {replaceMode && (
                <div className="flex items-start gap-3 p-3.5 rounded-lg bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                    <strong>Replace mode:</strong> Uploading will <strong>delete all existing offshore rate rows</strong> for <span className="font-mono">{uploadTarget}</span> and replace them with the rows in this file. This cannot be undone.
                  </p>
                </div>
              )}
